...
from api.models import TransferenciaRealizadaConvenio
...
router = APIRouter(
    prefix="/transferencia_realizada_convenio",
    tags=["Transferência Realizada Convênio"],
)

@router.get("/{convenio_id}/arquivo/")
async def obter_arquivo_transferencia_realizada(request: Request, convenio_id: UUID = Path(...)):
    try:
        convenio = await TransferenciaRealizadaConvenio.get(id=convenio_id).only('arquivo', 'numero_convenio', 'ano_convenio')
...
async def obter_transferencia_realizada(request: Request, convenio_id: UUID = Path(...)):
    try:
        convenio = await TransferenciaRealizadaConvenio.get(id=convenio_id)
...
async def exportar_transferencias_realizadas(
    request: Request,
    ...
    try:
        query = TransferenciaRealizadaConvenio.filter(estabelecimento=estabelecimento_id)
        if beneficiario:
        ...
        convenios = await query.only(
            'beneficiario', 'numero_convenio', 'ano_convenio', 'objeto',
        ...
        return await exportar_em_formato(
            ...
            rows=rows,
            filename_prefix=f"transferencia_realizada_convenio_{date.today()}",
        )
    except Exception as e:
        raise BadRequestException(str(e))
...
async def listar_transferencias_realizadas(
    request: Request,
    estabelecimento_id: UUID = Path(...),
    beneficiario: Optional[str] = Query(None),
    numero_convenio: Optional[str] = Query(None),
...
@router.post("/")
async def criar_transferencia_realizada(
    request: Request,
    ...
    try:
        ...
        convenio = await TransferenciaRealizadaConvenio.create(
...
async def atualizar_transferencia_realizada(
    request: Request,
    ...
    try:
        ...
        if beneficiario is not None:
            convenio.beneficiario = beneficiario
...
async def deletar_transferencia_realizada(request: Request, convenio_id: UUID = Path(...)):
    try:
        convenio = await TransferenciaRealizadaConvenio.get(id=convenio_id)

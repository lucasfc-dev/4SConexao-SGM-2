...
import { cadastrarDocPublicacao, editarDocPublicacao, excluirDocPublicacao, listarDocsPublicacao, obterDocPublicacao } from '../api/doc_publicacao/doc_publicacao'
import { useAuth } from '@/app/context/AuthContext'

export default function RelacaoDeLicitantesEOuContratadosSancionadosAdministrativamente() {
    ...
    const recarregarRegistros = async (filtrosParaPesquisa = {}, pagina = 1) => {
        if (user && user.estabelecimento) {
            try {
                ...
                if (filtrosParaPesquisa.descricao) filtrosAPI.descricao = filtrosParaPesquisa.descricao
                ...
                const registrosData = await listarDocsPublicacao(
                    'licitantes_sancionados', 
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!editando) {
                ...
                const dadosEnvio = {
                    categoria: 'licitantes_sancionados',
                ...
                await cadastrarDocPublicacao(dadosEnvio, 'licitantes_sancionados')
                toast.success('Relação de licitantes sancionados cadastrada com sucesso!')
            } else {
                ...
                await editarDocPublicacao(registroSelecionado.id, dadosEnvio)
                toast.success('Relação de licitantes sancionados atualizada com sucesso!')
            }
            ...
            toast.error('Erro ao salvar relação de licitantes sancionados: ' + error.message)
            console.error(error)
        } finally {
            setLoading(false)
        }
    ...
    const handleDownload = async (registro) => {
        try {
            ...
            const blob = await obterDocPublicacao(registro.id)
    ...
    const handleDeletar = async (registro) => {
        if (window.confirm('Tem certeza que deseja deletar este documento?')) {
            try {
                setLoading(true)
                await excluirDocPublicacao(registro.id)
        ...
        <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
                ...
                {mostrarFormulario ? (
                    <>
                        <h1 className="text-lg font-bold mb-4">
                            Cadastro de Relação de Licitantes e/ou Contratados Sancionados
                        </h1>
                    ...
                    <>
                        <h1 className="text-lg font-bold mb-4">Relação de Licitantes e/ou Contratados Sancionados</h1>
            ...
            {modalEdicao && registroSelecionado && (
                <Modal
                    ...
                    title={`Editar Relação de Licitantes Sancionados - ${registroSelecionado.titulo}`}
                >

...
export default function PlanoAnualContratacao() {
        ...
        if (user && user.estabelecimento) {
            try {
                ...
                const registrosData = await listarDocumentosNumerados(
                    ...
                    { offset, limit: itensPorPagina, filters: filtrosAPI }
                )
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!editando) {
                ...
                await criarDocumentoNumerado('plano_anual_contratacao', dadosEnvio)
                toast.success('Plano Anual de Contratação cadastrado com sucesso!')
            } else {
                ...
                toast.success('Plano Anual de Contratação atualizado com sucesso!')
            }
            ...
            toast.error('Erro ao salvar Plano Anual de Contratação: ' + error.message)
            console.error(error)
        } finally {
            setLoading(false)
        }
    ...
    const handleDeletar = async (registro) => {
        if (window.confirm('Tem certeza que deseja deletar este Plano Anual de Contratação?')) {
            try {
                setLoading(true)
                await excluirDocumentoNumerado(registro.id)
                await recarregarRegistros(filtros, paginaAtualfiltros, paginaAtual)
                toast.success('Plano Anual de Contratação deletado com sucesso!')
            } catch (error) {
                toast.error('Erro ao deletar Plano Anual de Contratação: ' + error.message)
                console.error(error)
            } finally {
                setLoading(false)
            }
        ...
        <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
                ...
                {mostrarFormulario ? (
                    <>
                        <h1 className="text-lg font-bold mb-4">
                            Cadastro de Plano Anual de Contratação
                        </h1>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <textarea
                                        ...
                                        onChange={handleInputChange}
                                        rows={4}
                                        placeholder="Descreva o plano de contratação, objetivos, estratégias, etc."
                                        className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none"
                                    />
                                </div>
                    ...
                    <>
                        <h1 className="text-lg font-bold mb-4">Lista de Planos Anuais de Contratação</h1>
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td colSpan="6" className="py-3 px-6 text-center text-gray-500">
                                                Nenhum Plano Anual de Contratação encontrado
                                            </td>
                                        </tr>
                                    </tfoot>
                                ) : (
            ...
            {modalEdicao && registroSelecionado && (
                <Modal
                    ...
                    title={`Editar Plano Anual de Contratação - ${registroSelecionado.num_doc}`}
                >

...
const CATEGORIA = 'conselho_saude'
...
export default function ConselhoMunicipalSaude() {
    ...
    const [filtros, setFiltros] = useState({ numeroDocumento: '', titulo: '', dataInicial: '', dataFinal: '' })
    const [paginaAtual, setPaginaAtual] = useState(1)
    const [itensPorPagina, setItensPorPagina] = useState(10)
    const [totalRegistros, setTotalRegistros] = useState(0)

    const [formData, setFormData] = useState({
        numeroDocumento: '', ano: '', titulo: '', descricao: '', dataPublicacao: '', arquivo: null
    })

    const resetForm = () => setFormData({ numeroDocumento: '', ano: '', titulo: '', descricao: '', dataPublicacao: '', arquivo: null })
    ...
    const recarregarRegistros = async (filtrosParaPesquisa = {}, pagina = 1) => {
        if (user && user.estabelecimento) {
            try {
                ...
                const registrosData = await listarDocumentosNumerados(CATEGORIA, user.estabelecimento.id, { offset, limit: itensPorPagina, filters: filtrosAPI })
    ...
    useEffect(() => { recarregarRegistros() }, [user])
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true)
        try {
            const dadosEnvio = { dataPublicacao: formData.dataPublicacao, titulo: formData.titulo, descricao: formData.descricao, numDoc: formData.numeroDocumento, arquivo: formData.arquivo }
            if (!editando) {
                await criarDocumentoNumerado(CATEGORIA, dadosEnvio)
    ...
    const handleEditar = (registro) => {
        ...
        setFormData({ numeroDocumento: registro.num_doc || '', ano: registro.ano || '', titulo: registro.titulo || '', descricao: registro.descricao || '', dataPublicacao: registro.data_publicacao || '', arquivo: null })
        setEditando(true)
        setModalEdicao(true)
    }
    ...
    const handleLimparFiltros = async () => {
        const filtrosLimpos = { numeroDocumento: '', titulo: '', dataInicial: '', dataFinal: '' }
    ...
    const camposFormulario = (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="numeroDocumento" value={formData.numeroDocumento} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div>
                <label className="block text-sm font-medium mb-1">Ano *</label>
                <input type="number" name="ano" value={formData.ano} onChange={handleInputChange} min="2000" max="2030" className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div className="md:col-span-2">
                <label className="block text-sm font-medium mb-1">Título do Documento *</label>
                <input type="text" name="titulo" value={formData.titulo} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div>
                <label className="block text-sm font-medium mb-1">Data de Publicação *</label>
                <input type="date" name="dataPublicacao" value={formData.dataPublicacao} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div className="md:col-span-2">
                <label className="block text-sm font-medium mb-1">Descrição</label>
                <textarea name="descricao" value={formData.descricao} onChange={handleInputChange} rows={3} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div className="md:col-span-2">
    ...
    return (
        <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
            ...
            <div className="flex flex-col w-full mx-auto bg-white shadow-lg rounded-lg p-4">
                {mostrarFormulario ? (
                    <>
                        <h1 className="text-lg font-bold mb-4">Cadastro de Documentos - Conselho Municipal de Saúde</h1>
                    ...
                    <>
                        <h1 className="text-lg font-bold mb-4">Documentos - Conselho Municipal de Saúde</h1>
                        <div className="bg-gray-50 rounded p-4 mb-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                                <div>
                                    <label className="block text-sm font-medium mb-1">Número</label>
                                    <input type="text" value={filtros.numeroDocumento} onChange={(e) => handleFiltroChange('numeroDocumento', e.target.value)} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Filtrar por número..." />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1">Título</label>
                                    <input type="text" value={filtros.titulo} onChange={(e) => handleFiltroChange('titulo', e.target.value)} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Filtrar por título..." />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1">Data Inicial</label>
                                    <input type="date" value={filtros.dataInicial} onChange={(e) => handleFiltroChange('dataInicial', e.target.value)} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1">Data Final</label>
                                    <input type="date" value={filtros.dataFinal} onChange={(e) => handleFiltroChange('dataFinal', e.target.value)} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
                                </div>
                            </div>
                            <div className="flex gap-2 justify-end mt-4">
                                <button type="button" onClick={handleLimparFiltros} disabled={loading} className="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 transition-colors disabled:opacity-50 flex items-center gap-2">Limpar</button>
                        ...
                        <div>
                            <table className="min-w-full bg-white mt-4 shadow-md rounded-lg">
                                ...
                                {loading ? (
                                    <tfoot><tr><td colSpan="6" className="py-3 px-6 text-center"><div className="flex justify-center items-center"><FiLoader size={24} className="animate-spin text-4xl text-azul_escuro" /></div></td></tr></tfoot>
                                ) : registros.length === 0 ? (
                                    <tfoot><tr><td colSpan="6" className="py-3 px-6 text-center text-gray-500">Nenhum documento encontrado</td></tr></tfoot>
                                ) : (
                                    <tbody className="text-gray-800 text-sm font-light">
                                        {registros.map((registro) => (
                                            <tr key={registro.id} className="border-b border-gray-200 hover:bg-gray-100">
                                                <td className="py-3 px-6 text-left">{registro.num_doc}</td>
                                                <td className="py-3 px-6 text-left">{registro.ano}</td>
                                                <td className="py-3 px-6 text-left">{registro.titulo}</td>
                                                <td className="py-3 px-6 text-left max-w-xs"><DescricaoTruncada texto={registro.descricao} maxLength={80} /></td>
                                                <td className="py-3 px-6 text-left">{registro.data_publicacao ? registro.data_publicacao.split('-').reverse().join('/') : '-'}</td>
                        ...
                        {totalRegistros > 0 && (
                            <PaginacaoAvancada paginaAtual={paginaAtual} totalItens={totalRegistros} itensPorPagina={itensPorPagina} onMudarPagina={handleMudancaPagina} onMudarItensPorPagina={(novoLimite) => { setItensPorPagina(novoLimite); setPaginaAtual(1); recarregarRegistros(filtros, 1) }} />
                        )}
                    </>
                )}
            </div>
            ...
            {modalEdicao && registroSelecionado && (
                <Modal isOpen={modalEdicao} onClose={() => { setModalEdicao(false); setEditando(false); resetForm(); setRegistroSelecionado(null) }} title={`Editar Documento - ${registroSelecionado.num_doc}`}>

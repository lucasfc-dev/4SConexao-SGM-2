...
import { cadastrarTransferenciaRecebidaConvenio, editarTransferenciaRecebidaConvenio, excluirTransferenciaRecebidaConvenio, listarTransferenciasRecebidasConvenios, obterArquivoTransferenciaRecebida } from '../api/transferencias_recebidas_convenios/transferencias_recebidas_convenios'
import { useAuth } from '@/app/context/AuthContext'
import { formatarMoeda, formatarMoedaInput, desformatarMoeda, numeroParaInput } from '@/app/utils/formatarMoeda'

export default function TransferenciasRecebidasConvenios() {
    ...
    const [filtros, setFiltros] = useState({
        orgaoRepassador: '', numeroConvenio: '', anoConvenio: '',
    ...
    const [formData, setFormData] = useState({
        orgaoRepassador: '', numeroConvenio: '', anoConvenio: '', objeto: '',
    ...
    const recarregarRegistros = async (filtrosParaPesquisa = {}, pagina = 1) => {
        if (user && user.estabelecimento) {
            try {
                setLoading(true)
                const filtrosAPI = {}
                if (filtrosParaPesquisa.orgaoRepassador) filtrosAPI.orgao_repassador = filtrosParaPesquisa.orgaoRepassador
                ...
                const registrosData = await listarTransferenciasRecebidasConvenios(user.estabelecimento.id, pagina, itensPorPagina, filtrosAPI)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const dadosEnvio = {
                orgaoRepassador: formData.orgaoRepassador,
            ...
            if (!editando) {
                await cadastrarTransferenciaRecebidaConvenio(dadosEnvio)
                toast.success('Transferência cadastrada com sucesso!')
            } else {
                await editarTransferenciaRecebidaConvenio(registroSelecionado.id, dadosEnvio)
    ...
    const handleEditar = (registro) => {
        ...
        setFormData({
            orgaoRepassador: registro.orgao_repassador || '',
    ...
    const handleDownload = async (registro) => {
        try {
            setLoading(true)
            const blob = await obterArquivoTransferenciaRecebida(registro.id)
            if (blob && blob.size > 0) {
    ...
    const handleDeletar = async (registro) => {
        if (window.confirm('Tem certeza que deseja deletar esta transferência recebida?')) {
            try {
                setLoading(true)
                await excluirTransferenciaRecebidaConvenio(registro.id)
    ...
    const handleLimparFiltros = async () => {
        const filtrosLimpos = { orgaoRepassador: '', numeroConvenio: '', anoConvenio: '', dataInicioVigenciaInicial: '', dataInicioVigenciaFinal: '', dataFimVigenciaInicial: '', dataFimVigenciaFinal: '' }
    ...
    const camposFormulario = (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
                <label className="block text-sm font-medium mb-1">Órgão Repassador *</label>
                <input type="text" name="orgaoRepassador" value={formData.orgaoRepassador} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div>
    ...
    return (
        <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
            ...
            <div className="flex flex-col w-full mx-auto bg-white shadow-lg rounded-lg p-4">
                {mostrarFormulario ? (
                    <>
                        <h1 className="text-lg font-bold mb-4">Cadastro de Transferências Recebidas Convênios</h1>
                    ...
                    <>
                        <h1 className="text-lg font-bold mb-4">Transferências Recebidas Convênios</h1>
                        <div className="bg-gray-50 rounded p-4 mb-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                                <div>
                                    <label className="block text-sm font-medium mb-1">Órgão Repassador</label>
                                    <input type="text" value={filtros.orgaoRepassador} onChange={(e) => handleFiltroChange('orgaoRepassador', e.target.value)} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Filtrar por órgão..." />
                                </div>
                                <div>
                            ...
                            <table className="min-w-full bg-white mt-4 shadow-md rounded-lg">
                                <thead>
                                    <tr className="bg-azul_escuro bg-opacity-90 text-branco_cinza uppercase text-sm leading-normal">
                                        <th className="py-3 px-6 text-left">Órgão Repassador</th>
                                    ...
                                    <tbody className="text-gray-800 text-sm font-light">
                                        {registros.map((registro) => (
                                            <tr key={registro.id} className="border-b border-gray-200 hover:bg-gray-100">
                                                <td className="py-3 px-6 text-left">{registro.orgao_repassador}</td>

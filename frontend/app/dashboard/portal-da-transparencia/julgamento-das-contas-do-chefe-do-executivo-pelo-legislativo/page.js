...
import { cadastrarJulgamentoContas, editarJulgamentoContas, excluirJulgamentoContas, listarJulgamentoContas, obterArquivoJulgamentoContas } from '../api/julgamento_contas/julgamento_contas'
import { useAuth } from '@/app/context/AuthContext'

export default function JulgamentoContasExecutivo() {
    ...
    const recarregarRegistros = async (filtrosParaPesquisa = {}, pagina = 1) => {
        if (user && user.estabelecimento) {
            try {
                ...
                if (filtrosParaPesquisa.titulo) filtrosAPI.numero = filtrosParaPesquisa.titulo
                ...
                const registrosData = await listarJulgamentoContas(user.estabelecimento.id, filtrosAPI)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!editando) {
                await cadastrarJulgamentoContas(formData)
                toast.success('Julgamento de contas cadastrado com sucesso!')
            } else {
                await editarJulgamentoContas(registroSelecionado.id, formData)
                toast.success('Julgamento de contas atualizado com sucesso!')
            }
            ...
            toast.error('Erro ao salvar julgamento: ' + error.message)
            console.error(error)
        } finally {
            setLoading(false)
        }
    ...
    const handleEditar = (registro) => {
        ...
        setFormData({
            ...
            anoProcesso: registro.ano_processo || '',
    ...
    const handleDeletar = async (registro) => {
        if (window.confirm('Tem certeza que deseja deletar este documento?')) {
            try {
                setLoading(true)
                await excluirJulgamentoContas(registro.id)
                await recarregarRegistros(filtros, paginaAtual)
    ...
    const handleDownload = async (registro) => {
        try {
            ...
            const blob = await obterArquivoJulgamentoContas(registro.id)
        ...
        <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
                ...
                {mostrarFormulario ? (
                    <>
                        <h1 className="text-lg font-bold mb-4">
                            {editando ? 'Editar Julgamento de Contas' : 'Cadastro de Julgamento das Contas do Chefe do Executivo'}
                        </h1>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option value="Regular com Ressalvas">Regular com Ressalvas</option>
                                        <option value="Tramitando">Tramitando</option>
                                        <option value="Iliquidáveis">Iliquidáveis</option>
                                    </select>
                                </div>
                    ...
                    <>
                        <h1 className="text-lg font-bold mb-4">Julgamento das Contas do Chefe do Executivo pelo Legislativo</h1>
                                    ...
                                    <tbody className="text-gray-800 text-sm font-light">
                                        {registrosPaginados.map((registro) => (
                                            <tr key={registro.id} className="border-b border-gray-200 hover:bg-gray-100">
                                                ...
                                                <td className="py-3 px-6 text-left">
                                                    {registro.data_resultado ? registro.data_resultado.split('-').reverse().join('/') : '-'}
                                                </td>
                                                <td className="py-3 px-6 text-left">
                                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${registro.status === 'Regular' ? 'bg-green-100 text-green-800' :
                                                        registro.status === 'Irregular' ? 'bg-red-100 text-red-800' :
                                                            registro.status === 'Regular com Ressalvas' ? 'bg-yellow-100 text-yellow-800' :
                                                                registro.status === 'Tramitando' ? 'bg-blue-100 text-blue-800' :
                                                                    registro.status === 'Iliquidáveis' ? 'bg-gray-100 text-gray-800' :
            ...
            {modalEdicao && registroSelecionado && (
                <Modal
                    ...
                    title={`Editar Julgamento - ${registroSelecionado.numero}`}
                >

...
export default function IncentivosProjutosCulturais() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!editando) {
                ...
                await criarDocumentoNumerado('incentivos_culturais', dadosEnvio)
                toast.success('Incentivo cultural cadastrado com sucesso!')
            } else {
                // Editar incentivo cultural existente
                const dadosEnvio = {
                    dataPublicacao: formData.dataPublicacao,
                    titulo: formData.titulo,
                    descricao: formData.descricao,
                ...
                toast.success('Incentivo cultural atualizado com sucesso!')
            }
            ...
            toast.error('Erro ao salvar incentivo cultural: ' + error.message)
            console.error(error)
        } finally {
            setLoading(false)
        }
    ...
    const handleDeletar = async (registro) => {
        if (window.confirm('Tem certeza que deseja deletar este incentivo cultural?')) {
            try {
                setLoading(true)
                await excluirDocumentoNumerado(registro.id)
                await recarregarRegistros(filtros, paginaAtual)
                toast.success('Incentivo cultural deletado com sucesso!')
            } catch (error) {
                toast.error('Erro ao deletar incentivo cultural: ' + error.message)
                console.error(error)
            } finally {
                setLoading(false)
            }
        ...
        <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
                ...
                {mostrarFormulario ? (
                    <>
                        <h1 className="text-lg font-bold mb-4">
                            Cadastro de Incentivos a Projetos Culturais
                        </h1>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <textarea
                                        ...
                                        onChange={handleInputChange}
                                        rows={4}
                                        placeholder="Descreva o projeto cultural, beneficiários, objetivos, etc."
                                        className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none"
                                    />
                                </div>
                    ...
                    <>
                        <h1 className="text-lg font-bold mb-4">Lista de Incentivos a Projetos Culturais</h1>
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td colSpan="6" className="py-3 px-6 text-center text-gray-500">
                                                Nenhum incentivo cultural encontrado
                                            </td>
                                        </tr>
                                    </tfoot>
                                ) : (
            ...
            {modalEdicao && registroSelecionado && (
                <Modal
                    ...
                    title={`Editar Projeto Cultural - ${registroSelecionado.num_doc}`}
                >

...
export default function DocumentosEPublicacoesMeioAmbiente() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!editando) {
                ...
                await criarDocumentoNumerado('documentos_meio_ambiente', dadosEnvio)
                toast.success('Documento de meio ambiente cadastrado com sucesso!')
            } else {
                ...
                toast.success('Documento de meio ambiente atualizado com sucesso!')
            }
            ...
            toast.error('Erro ao salvar documento de meio ambiente: ' + error.message)
            console.error(error)
        } finally {
            setLoading(false)
        }
    ...
    const handleDeletar = async (registro) => {
        if (window.confirm('Tem certeza que deseja deletar este documento de meio ambiente?')) {
            try {
                setLoading(true)
                await excluirDocumentoNumerado(registro.id)
                await recarregarRegistros(filtros, paginaAtual)
                toast.success('Documento de meio ambiente deletado com sucesso!')
            } catch (error) {
                toast.error('Erro ao deletar documento de meio ambiente: ' + error.message)
                console.error(error)
            } finally {
                setLoading(false)
            }
        ...
        <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
                ...
                {mostrarFormulario ? (
                    <>
                        <h1 className="text-lg font-bold mb-4">
                            Cadastro de Documentos e Publicações de Meio Ambiente
                        </h1>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <textarea
                                        ...
                                        onChange={handleInputChange}
                                        rows={4}
                                        placeholder="Descreva o conteúdo do documento, objetivos ambientais, programas, etc."
                                        className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:outline-none"
                                    />
                                </div>
                    ...
                    <>
                        <h1 className="text-lg font-bold mb-4">Lista de Documentos e Publicações de Meio Ambiente</h1>
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td colSpan="6" className="py-3 px-6 text-center text-gray-500">
                                                Nenhum documento de meio ambiente encontrado
                                            </td>
                                        </tr>
                                    </tfoot>
                                ) : (
            ...
            {modalEdicao && registroSelecionado && (
                <Modal
                    ...
                    title={`Editar Documento - ${registroSelecionado.num_doc}`}
                >

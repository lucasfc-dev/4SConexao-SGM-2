...
import { useLicitacoes } from "../context/LicitacoesContext"
...
export default function Licitacoes() {
  ...
  const { listaLicitacoes, totalLicitacoes, loadingLicitacoes, fetchLicitacoes } = useLicitacoes()
  ...
  // Handler para teclas pressionadas nos campos monetários
  ...
  const inicializarValoresEdicao = (licitacao) => {
    if (licitacao?.valor_estimado) {
      const valorEstimadoFormatado = parseFloat(licitacao.valor_estimado).toFixed(2).replace('.', ',')
    ...
    if (licitacao?.valor_vencedor) {
      const valorVencedorFormatado = parseFloat(licitacao.valor_vencedor).toFixed(2).replace('.', ',')
  ...
  const listaColunasLicitacao = [
    { nomeColuna: 'Número do processo' },
    { nomeColuna: 'Órgão' },
    { nomeColuna: 'Modalidade' },
    { nomeColuna: 'Situação' },
  ...
  const getDadosResponsavel = async (licitacao) => {
    try {
      const estabelecimento = user.estabelecimento.id
      const response = await fetch(`${acUrl}/comissao/responsavel/?estabelecimento=${estabelecimento}`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(jsonError.error || jsonError.detail || 'Erro ao obter dados do responsável')
      }
    }
    catch (error) {
  ...
  const buildApiFilters = useCallback(() => {
    ...
    if (filtros.modalidade) f['modalidade'] = filtros.modalidade
  ...
  const refreshLicitacoes = useCallback(() => {
    const offset = (paginaAtual - 1) * itensPorPagina
    return fetchLicitacoes({ limit: itensPorPagina, offset, filters: buildApiFilters() })
  }, [paginaAtual, itensPorPagina, fetchLicitacoes, buildApiFilters])

  useEffect(() => {
    refreshLicitacoes()
  }, [paginaAtual, itensPorPagina])

  const hasActiveFilters = filtros.numero || filtros.modalidade || filtros.objeto ||
  ...
  const handleVisualizar = async (licitacao) => {
    ...
    try {
      await getDadosResponsavel(licitacao)
  ...
  const handleDeletar = async (licitacao) => {
    try {
      const response = await fetch(`${acUrl}/licitacao/${licitacao.id}/`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        }
      ...
      if (response.ok) {
        ...
        await refreshLicitacoes()
      }
      else {
        const jsonError = await response.json()
        toast.error(jsonError.error || jsonError.detail || 'Erro ao deletar licitação')
  ...
  const handleEditar = async (licitacao) => {
    setLicitacaoSelecionada(licitacao)
    inicializarValoresEdicao(licitacao)
    setEditando(true)
  }

  const editarLicitacao = async (e) => {
    e.preventDefault()
    const formDataLicitacao = new FormData(e.target)
    const licitacao = Object.fromEntries(formDataLicitacao.entries())
    ...
    const licitacaoSemVazios = removerCamposVazios(licitacao)

    // Converte valores formatados de volta para número
    if (licitacaoSemVazios.valor_estimado) {
      licitacaoSemVazios.valor_estimado = valorParaNumero(licitacaoSemVazios.valor_estimado)
    }
    if (licitacaoSemVazios.valor_vencedor) {
      licitacaoSemVazios.valor_vencedor = valorParaNumero(licitacaoSemVazios.valor_vencedor)
    }

    try {
      const response = await fetch(`${acUrl}/licitacao/${licitacaoSelecionada.id}/`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(licitacaoSemVazios),
      })
      if (response.ok) {
        await response.json()
        ...
        const jsonError = await response.json()
        toast.error(jsonError.error || jsonError.detail || 'Erro ao editar licitação')
        throw new Error('Erro ao editar licitação')
      }
    }
    catch (error) {
      toast.error('Erro ao editar licitação:', error)
    }
  }

  const handleCadastrarDocs = async (licitacao, files) => {
    const fileData = new FormData()
    fileData.append('target_id', licitacao.id)
    fileData.append('target_type', 'licitacao')
    ...
    if (responseDocs.ok) {
      ...
      setDocumentosLicitacao(prev => [...prev, novoDocumento[0]])
    }
    else {
      const jsonError = await response.json()
      toast.error(jsonError.error || jsonError.detail || 'Erro ao adicionar documento')
  ...
  const handleCadastro = async (event) => {
    ...
    try {
      const response = await fetch(`${acUrl}/licitacao/`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        try {
          const responseDocs = await handleCadastrarDocs(novaLicitacao, files)
          if (responseDocs.ok) {
            await refreshLicitacoes()
            ...
            const jsonError = await response.json()
            toast.error(jsonError.error || jsonError.detail || 'Erro ao adicionar licitação')
            throw new Error('Erro ao adicionar licitação')
          }
        }
        catch (error) {
          throw new Error('Erro ao enviar arquivos da licitação')
        }
      }
    }
    catch (error) {
  ...
  const handleExcluirDoc = async (doc) => {
    try {
      ...
      if (response.ok) {
        setDocumentosLicitacao(prev => prev.filter(d => d.id !== doc.id))
  ...
  const handleGerarCertificado = async (licitacao) => {
    try {
      const response = await fetch(`${acUrl}/licitacao/${licitacao.id}/certificado/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleDownloadCertificado = async (licitacaoId) => {
    try {
      const response = await fetch(`${acUrl}/licitacao/${licitacaoId}/certificado/content/`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
        }
      ...
      if (response.ok) {
        ...
        const filename = match ? match[1] : `Certificado de Publicação de Licitação ${licitacaoId}.pdf`
        const blob = await response.blob()
        const url = window.URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
  ...
  const handleExcluirCertificado = async (licitacaoId) => {
    try {
      const response = await fetch(`${acUrl}/licitacao/${licitacaoId}/certificado/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleMudarItensPorPagina = (novosItens) => {
    ...
    setPaginaAtual(1) // Reset para primeira página quando mudar itens por página
  }

  const acoesLicitacao = [
    {
      nome: <FaEdit size={28} className="text-green-800 hover:text-green-900 transition-colors" />,
      handler: handleEditar,
    },
    {
      nome: <FaEye size={28} className="text-azul_escuro hover:text-azul_escuro*8 transition-colors" />,
  ...
  return (
    <section className="flex flex-col flex-grow overflow-auto text-azul_escuro gap-4 bg-gradient-to-br from-gray-200 to-gray-300 p-4">
      ...
      <div className="flex flex-col w-full mx-auto bg-white shadow-lg rounded-lg p-4">
        {mostrarFormulario ? (
          <>
            <h1 className="text-lg font-bold mb-2">Cadastro de Licitações</h1>
            <form onSubmit={handleCadastro} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-2 w-full mx-auto text-azul_escuro">
                ...
                <div>
                  <label className="block text-sm font-medium">Natureza do Procedimento</label>
                  <select name="natureza_procedimento" required className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all">
          ...
          <div className="flex flex-col w-full mx-auto rounded-lg">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
              <h2 className="text-azul_escuro text-xl font-bold">Licitações Cadastradas</h2>
            </div>
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-azul_escuro bg-opacity-90 text-branco_cinza uppercase text-sm leading-normal">
                    ...
                    <th className="py-3 px-4 text-left min-w-[140px]">Modalidade</th>
                ...
                <tbody className="text-gray-800 text-sm font-light">
                  {loadingLicitacoes ? (
                    ...
                    listaLicitacoes.map((licitacao) => (
                      <tr key={licitacao.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                        <td className="py-4 px-4 font-medium text-azul_escuro">{licitacao.num_processo}</td>
                        <td className="py-4 px-4">{licitacao.orgao.nome}</td>
                        <td className="py-4 px-4">{licitacao.modalidade.nome}</td>
                        <td className="py-4 px-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                            ['aberta', 'homologada', 'em_andamento'].some(s => licitacao.situacao?.toLowerCase().includes(s))
                              ? 'bg-green-100 text-green-800'
                              : 'bg-yellow-100 text-yellow-800'
                          }`}>
                            {licitacao.situacao?.toUpperCase()}
                          </span>
                        </td>
                        <td className="py-4 px-4">{formatarData(licitacao.pub_date)}</td>
                        <td className="py-4 px-4">
                          <p className="truncate max-w-[360px]" title={licitacao.objeto}>{licitacao.objeto}</p>
                        </td>
                        <td className="py-4 px-4">
                          <div className="flex justify-center space-x-2">
                            {acoesLicitacao.map((acao, index) => (
                              <button
                                key={index}
                                onClick={() => acao.handler(licitacao)}
                            ...
                            {licitacao.certificado_publicacao ? (
                              <>
                                <button
                                  onClick={() => handleDownloadCertificado(licitacao.id)}
                                  className="p-1 hover:scale-110 transition-transform"
                                ...
                                <button
                                  onClick={() => handleExcluirCertificado(licitacao.id)}
                                  className="p-1 hover:scale-110 transition-transform"
                              ...
                              <button
                                onClick={() => handleGerarCertificado(licitacao)}
                                className="p-1 hover:scale-110 transition-transform"
                    ...
                    <tr>
                      <td colSpan="7" className="py-12 text-center">
                        <div className="flex flex-col items-center text-gray-500">
                          ...
                          <p className="text-lg font-medium mb-2">Nenhuma licitação encontrada</p>
                          <p className="text-sm">
                            {totalLicitacoes === 0 ? 'Nenhuma licitação foi cadastrada ainda.' : 'Tente ajustar os filtros da pesquisa.'}
                          </p>
                        </div>
                      </td>
                    </tr>
            ...
            <div className={`space-y-4 ${isDesktop ? 'hidden' : 'block'}`}>
              {loadingLicitacoes ? (
                <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
                  ...
                  <p className="text-gray-500 mt-4">Carregando licitações...</p>
                </div>
              ) : listaLicitacoes.length > 0 ? (
                listaLicitacoes.map((licitacao) => (
                  <div key={licitacao.id} className="bg-white rounded-lg shadow-md border border-gray-200 p-4 hover:shadow-lg transition-shadow">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex flex-col">
                        <h3 className="text-lg font-bold text-azul_escuro">Licitação Nº {licitacao.num_processo}</h3>
                        ...
                        {licitacao.certificado_publicacao ? (
                          <>
                            ...
                            <button
                              onClick={() => handleExcluirCertificado(licitacao.id)}
                              className="p-2 hover:scale-110 transition-transform rounded-full hover:bg-gray-100"
                              title="Excluir Certificado de Publicação"
                            >
                              <FaTrash size={22} className="text-red-500 hover:text-red-700 transition-colors" />
                            </button>
                          </>
                        ) : (
                          <button
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        ...
                        <p className="text-gray-800 mt-1">{licitacao.orgao.nome}</p>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div>
                          <span className="font-semibold text-gray-600">Modalidade:</span>
                          <p className="text-gray-800">{licitacao.modalidade.nome}</p>
                        </div>
                        <div>
                          <span className="font-semibold text-gray-600">Data de Publicação:</span>
                          <p className="text-gray-800">{formatarData(licitacao.pub_date)}</p>
                        </div>
                      </div>
                      <div className="mt-3">
                        <span className="font-semibold text-gray-600">Objeto:</span>
                        <div className="mt-1">
                          {licitacao.objeto.length > 150 ? (
                            <details className="group">
                              <summary className="cursor-pointer text-gray-800 hover:text-azul_escuro list-none">
                                <span>{licitacao.objeto.substring(0, 150)}...</span>
                              ...
                              <p className="mt-2 text-gray-800 leading-relaxed">{licitacao.objeto}</p>
                            </details>
                          ) : (
                ...
                <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
                  <div className="flex flex-col items-center text-gray-500">
                    ...
                    <p className="text-xl font-medium mb-2 text-gray-700">Nenhuma licitação encontrada</p>
                    <p className="text-gray-500 max-w-md">
                      {totalLicitacoes === 0
                        ? 'Nenhuma licitação foi cadastrada ainda. Use a aba "Cadastro" para criar a primeira licitação.'
                        : 'Não há licitações que correspondam aos filtros aplicados. Tente ajustar os critérios de pesquisa.'}
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-branco_cinza rounded-xl shadow-2xl w-full max-w-[80%] max-h-[80%] flex flex-col overflow-hidden">
            <header className="flex items-center justify-between bg-laranja_escuro text-white px-4 py-2 rounded-t-lg shrink-0">
              <h2 className="text-lg font-semibold">Detalhes da Licitação - {licitacaoSelecionada.num_processo}</h2>
              <CgClose
          ...
          <div className="p-6 overflow-y-auto">
            <div className="flex flex-wrap gap-4 bg-white p-4 shadow-lg rounded mb-6">
              <div className="flex flex-col w-full sm:w-1/2 lg:w-1/3">
                <strong>Órgão:</strong>
                <span className="break-words">{licitacaoSelecionada.orgao.nome}</span>
              </div>
              ...
              <div className="flex flex-col w-full sm:w-1/2 lg:w-1/3">
                ...
                <span className="break-words">{licitacaoSelecionada.modalidade.nome}</span>
              </div>
              ...
              <div className="flex flex-col w-full sm:w-1/2 lg:w-1/3">
                ...
                <span className="break-words">
                  {formatarData(licitacaoSelecionada.pub_date)}
                </span>
              </div>
              ...
              <div className="flex flex-col w-full sm:w-1/2 lg:w-1/3">
                ...
                <span className="break-words">
                  {formatarData(licitacaoSelecionada.homolog_date)}
                </span>
              </div>
              ...
              <div className="flex flex-col w-full sm:w-1/2 lg:w-1/3">
                ...
                <span className="break-words">
                  {licitacaoSelecionada.julg_date ? formatarData(licitacaoSelecionada.julg_date) : 'Não Julgado'}
                </span>
              </div>
              ...
              <div className="flex gap-4 items-center">
                <div className="relative text-sm inline-block cursor-pointer">
                  <label className="bg-azul_escuro text-white px-4 cursor-pointer py-2 rounded-md cursor-pointer hover:bg-laranja_escuro transition-all">
                    Adicionar Documento
                    <input
                      onChange={(e) => handleCadastrarDocs(licitacaoSelecionada, e.target.files)}
                      type="file"
                      name="files"
                      accept=".docx, .pdf"
      ...
      {editando && licitacaoSelecionada && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-branco_cinza rounded-xl shadow-2xl w-full max-w-[80%] max-h-[80%] flex flex-col overflow-hidden">
            <header className="flex items-center justify-between bg-laranja_escuro text-white px-4 py-2 rounded-t-lg shrink-0">
              <h2 className="text-lg font-semibold">Editar Licitação - {licitacaoSelecionada.num_processo}</h2>
              <CgClose
          ...
          <form onSubmit={(e) => editarLicitacao(e)} className="flex p-6 overflow-auto flex-col gap-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-2 overflow-auto w-full mx-auto text-azul_escuro">
              <div>
                <label className="block text-sm font-medium ">Órgão</label>
                <select
                  required
                  defaultValue={licitacaoSelecionada.orgao.id}
                  name="orgao"
                  className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all"
                >
                  <option value="">Selecione um órgão</option>
              ...
              <div>
                <label className="block text-sm font-medium">Número do Processo</label>
                <input required name="num_processo" defaultValue={licitacaoSelecionada.num_processo} type="text" className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all" />
              </div>

              <div>
                <label className="block text-sm font-medium">Modalidade</label>
                <select name="modalidade" required defaultValue={licitacaoSelecionada.modalidade.id} className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all">
                  {listaModalidades.map(modalidade => {
                    return (
                      <option key={modalidade.id} value={modalidade.id}>{modalidade.nome}</option>
                    )
              ...
              <div>
                <label className="block text-sm font-medium">Seção</label>
                <select name="secao" defaultValue={licitacaoSelecionada.secao.id} required className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all">
                  {listaSecoes.map(secao => {
                    return (
                      <option key={secao.id} value={secao.id}>{secao.nome}</option>
                    )
              ...
              <div>
                <label className="block text-sm font-medium">Regime de Execução</label>
                <select name="regime_execucao" required defaultValue={licitacaoSelecionada.regime_execucao} className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all">
                  <option value="NA">Não se Aplica</option>
                  <option value="global">Empreitada por preço global</option>
                  <option value="unitario">Empreitada por preço unitário</option>
                  <option value="integral">Empreitada integral</option>
              ...
              <div>
                <label className="block text-sm font-medium">Natureza do Procedimento</label>
                <select name="natureza_procedimento" required defaultValue={licitacaoSelecionada.natureza_procedimento} className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all">
                  <option value="normal">Normal</option>
                  <option value="registro_precos">Registro de Preços</option>
                  <option value="credenciamento_chamamento">Credenciamento/Chamamento</option>
                  <option value="adesao_registro_precos">Adesão de Registro de Preços</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Situação</label>
                <select name="situacao" required defaultValue={licitacaoSelecionada.situacao} className="p-3 border border-gray-300 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro focus:border-laranja_escuro transition-all shadow-sm bg-white">
                  <option value="">Selecione a situação</option>
                  <option value="em_andamento">Em Andamento</option>
                  <option value="aberta">Aberta</option>
                  <option value="julgada">Julgada</option>
              ...
              <div>
                <label className="block text-sm font-medium">Data de Publicação</label>
                <input required name="pub_date" defaultValue={licitacaoSelecionada.pub_date} type="date" className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all" />
              </div>

              <div>
                <label className="block text-sm font-medium">Data de Julgamento (Opcional)</label>
                <input name="julg_date" defaultValue={licitacaoSelecionada.julg_date} type="date" className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all" />
              </div>

              <div>
                <label className="block text-sm font-medium">Data de Homologação (Opcional)</label>
                <input name="homolog_date" defaultValue={licitacaoSelecionada.homolog_date} type="date" className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all" />
              </div>

              <div className="md:col-span-2">
                <label className="block text-sm font-medium">Objeto</label>
                <textarea required name="objeto" defaultValue={licitacaoSelecionada.objeto} rows="3" className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all"></textarea>
              </div>


              <div className="md:col-span-2">
                <label className="block text-sm font-medium">Fundamento Legal (Opcional)</label>
                <textarea name="fundamento_legal" defaultValue={licitacaoSelecionada.fundamento_legal} rows="2" className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-laranja_escuro transition-all"></textarea>
              </div>

...
import { FaDownload, FaEye, FaHandshake, FaCalendarAlt, FaList, FaUser } from "react-icons/fa"
...
export default function TransferenciasRealizadasConveniosIframe() {
    ...
    const getRegistros = async (resetCount, page = 1) => {
        ...
        try {
            ...
            const response = await fetch(`${transparenciaUrl}/transferencia_realizada_convenio/${estId}/?${queryParams}`)
            if (!response.ok) throw new Error('Erro ao obter transferências realizadas')
            ...
            console.error('Erro ao obter transferências realizadas:', error)
            toast.error('Erro ao carregar transferências realizadas')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDownload = async (registro) => {
        try {
            const response = await fetch(`${transparenciaUrl}/transferencia_realizada_convenio/${registro.id}/arquivo/`)
    ...
    const handleExport = async (formato) => {
        ...
        try {
            const response = await fetch(`${transparenciaUrl}/transferencia_realizada_convenio/${estId}/exportar/?type=${formato}&${queryParams}`)
            if (!response.ok) throw new Error('Falha ao exportar')
            const blob = await response.blob()
            const url = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = url
            a.download = `transferencias_realizadas_convenios.${formato}`
            document.body.appendChild(a)
            a.click()
            document.body.removeChild(a)
            URL.revokeObjectURL(url)
    ...
    return (
        <section className="flex flex-col text-azul_escuro flex-grow overflow-hidden bg-branco_cinza px-4 py-6 md:px-6 md:py-8 gap-6">
            ...
            <FormularioFiltrosAvancado
                titulo="Filtros de Transferências Realizadas por Convênios"
                filtros={filtros}
                onFiltroChange={handleFiltroChange}
                labelsMap={{}}
                onLimparFiltros={() => setFiltros({
                    beneficiario: '', numero_convenio: '', ano_convenio: '',
            ...
            <div className="flex flex-col bg-white rounded-2xl shadow-lg p-6 gap-2 md:p-8 border border-gray-100">
                <div className="flex justify-between items-center mb-2">
                    <h1 className="text-xl font-bold">Transferências Realizadas por Convênios</h1>
